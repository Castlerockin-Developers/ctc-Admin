...
const ViewCourse = ({ onUnassign, onEdit, onDelete }) => {
    // Dummy profile data
    const dummyCourse = {
        name: 'Jane Doe',
    ...
    const dummyChapters = [
        {
            ...
            questions: [
                { id: '1-1', text: 'What is JSX?', expectedOutput: 'JavaScript XML syntax' },
                { id: '1-2', text: 'How do you create a React component?', expectedOutput: 'Function or Class' },
            ],
        },
        {
            id: 2,
            ...
            questions: [
                { id: '2-1', text: 'Explain state vs props.', expectedOutput: 'State is local; props are external' },
                { id: '2-2', text: 'How to update state in functional components?', expectedOutput: 'useState hook' },
            ],
        },
    ];
                    ...
                    <div className='flex items-center gap-10'>
                        ...
                        <div className='profile-info-text'>
                            ...
                            <div className='created-at-date'>
                                Created At: {new Date(dummyCourse.createdAt).toLocaleDateString()}
                            </div>
                        </div>
                    </div>
                    {/* Right side: Buttons */}
                    <div className='profile-actions'>
                        <button onClick={() => onUnassign && onUnassign()} className='btn-unassign'>
                            Unassign
                        </button>
                        <button
                        ...
                        <button onClick={() => onDelete && onDelete()} className='btn-delete'>
                            Delete
                        </button>
                    </div>
                </div>
                ...
                <div className='chapters-column'>
                    ...
                    {dummyChapters.map((chapter) => (
                        <div key={chapter.id} className='chapter-item'>
                            ...
                            >
                                {chapter.name}
                                <span className='expand-icon'>{expandedChapter === chapter.id ? '▲' : '▼'}</span>
                            </div>
                    ...
                    <div className='student-list'>
                        {filteredStudents.length === 0 ? (
                            ...
                            filteredStudents.map((student) => (
                                <div
                                    key={student.id}
                                    className={`student-item ${student.completed ? 'completed' : 'not-completed'}`}
                                >
