...
const ManageExam = ({ onCreateNewExam, cacheAllowed, onEditExam }) => {
  ...
  const tableData = examsData
    ? examsData.map((exam) => ({
        id: exam.id,
        name: exam.name,
        startTime: new Date(exam.start_time).toLocaleString("en-US", {
          dateStyle: "short",
          timeStyle: "short",
          hour12: true,
        }),
        endTime: new Date(exam.end_time).toLocaleString("en-US", {
          dateStyle: "short",
          timeStyle: "short",
          hour12: true,
        }),
        startTimestamp: new Date(exam.start_time).getTime(),
        endTimestamp: new Date(exam.end_time).getTime(),
        attemptsAllowed: exam.attempts_allowed,
        status:
          new Date(exam.start_time) > new Date()
            ? "Upcoming"
            : exam.is_result_declared
            ? "Results Declared"
            ...
            <div className="m-btn-right flex sm:flex-row flex-col-reverse sm:justify-end justify-center items-center gap-2 w-full sm:w-auto">
              ...
              >
                <button className="filter-btn" onClick={toggleFilter}>
                  <FaFilter className="h-6 w-6 cursor-pointer text-white" />
                </button>
                {showFilter && (
                  <motion.div
                  ...
                  >
                    <ul className="py-2 text-white filter-btn-options">
                      <li
                        className={`px-4 py-3 hover:bg-[#535353] cursor-pointer ${
                          activeButton === "all" ? "font-semibold" : ""
                        }`}
                      ...
                      <li
                        className={`px-4 py-3 hover:bg-[#535353] cursor-pointer ${
                          activeButton === "active" ? "font-semibold" : ""
                        }`}
                      ...
                      <li
                        className={`px-4 py-3 hover:bg-[#535353] cursor-pointer ${
                          activeButton === "upcoming" ? "font-semibold" : ""
                        }`}
                      ...
                      <li
                        className={`px-4 py-3 hover:bg-[#535353] cursor-pointer ${
                          activeButton === "completed" ? "font-semibold" : ""
                        }`}
...
const ViewExam = ({ exam, onBack, onEditExam }) => {
  ...
  const handleViewExam = async (examSummary) => {
    try {
      const response = await authFetch(`/admin/exams/${examSummary.id}/`, {
        method: "GET",
      });
      if (!response.ok) {
      ...
      swal({
        title: "Error",
        text: "Failed to fetch exam details. Please try again.",
        icon: "error",
        background: "#1F1F1F",
        color: "#fff",
          ...
          <div className="viewexam-header">
            ...
            <div className="viewexam-header-btn">
              ...
              {!isExamCompleted(examDetails) && (
                <button className="viewexam-edit-btn" onClick={handleEditClick}>
                  Edit
                </button>
              )}
            </div>
            ...
            <div className="viewexam-viwer">
              <div className="viewexam-q">
                <div className="viewexam-viwer-header flex justify-between">
                  <p className="text-xl text-bold text-white leading-loose">
                    MCQ
                  </p>
                      ...
                      >
                        <div className="flex justify-between items-center w-full text-xl py-2">
                          <p className="text-white">
                            {index + 1}.{" "}
                            {section.section_name || "Sample Question"}
                          </p>
                          <p className="text-sm text-white whitespace-nowrap">
                            Timed: {section.is_timed ? "Yes" : "No"}{" "}
                            {section.is_timed &&
                              `| Time: ${section.section_time} min`}{" "}
                            | Total: {section.no_of_question}
                          </p>
                        </div>
                      </motion.div>
                    ))}
              ...
              <div className="viewexam-q">
                <div className="viewexam-viwer-header flex justify-between">
                  <p className="text-xl font-bold text-white leading-loose">
                    Coding
                  </p>
                  <p className="pb-0">
                ...
                <div className="viewexam-viwer-body flex justify-center">
                  <div className="viewexams-container pb-2">
                    {examDetails.selected_coding_questions?.map(
